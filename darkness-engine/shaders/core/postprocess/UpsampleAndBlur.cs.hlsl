...
Texture2D<float3> HigherResBuf;
Texture2D<float3> LowerResBuf;
sampler LinearBorder;
RWTexture2D<float3> Result;

cbuffer UpsampleAndBlurConstants
{
...
void BlurHorizontally(uint outIndex, uint leftMostIndex)
{
    ...
    Store1Pixel(outIndex, Blur9(s0, s1, s2, s3, s4, s5, s6, s7, s8));
    Store1Pixel(outIndex + 1, Blur9(s1, s2, s3, s4, s5, s6, s7, s8, s9));
}
...
void BlurVertically(uint2 pixelCoord, uint topMostIndex)
{
    ...
    Result[pixelCoord] = Blur9(s0, s1, s2, s3, s4, s5, s6, s7, s8);
}
