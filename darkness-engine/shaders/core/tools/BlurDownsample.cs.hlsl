...
void main(uint3 Gid : SV_GroupID, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
	...
	Store2Pixels(destIdx + 0, input[(ThreadUL + uint2(0, 0)) * 2].xyz, input[(ThreadUL + uint2(1, 0)) * 2].xyz);
	Store2Pixels(destIdx + 8, input[(ThreadUL + uint2(0, 1)) * 2].xyz, input[(ThreadUL + uint2(1, 1)) * 2].xyz);
