...
void main(uint3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	{
		uint bufferIndex = voxelList[dispatchThreadID.x];
			...
			//if (voxels[dispatchThreadID.xyz])
		{
			...
			uint3 gridPosition = decodeMorton(bufferIndex / 2);

			createVertices(
				cubeLocation * 24,
				((((float3(gridPosition) / voxelDepth) - float3(0.5, 0.5, 0.0)) * float3(-1.0f, -1.0f, -1.0f)) * 10),
				1.0f / voxelDepth * 5);
