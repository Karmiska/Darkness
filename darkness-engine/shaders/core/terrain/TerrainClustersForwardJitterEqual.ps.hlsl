...
float3 normalFromHeights(float3 height)
{
...
float textureBicubic(float2 texCoords)
{
	texCoords = texCoords * heightMapSize - 0.5;
	...
	offset *= heightMapTexelSize.xxyy;

	float sample0 = heightmap.SampleLevel(tex_sampler, offset.xz, 0).x;
	float sample1 = heightmap.SampleLevel(tex_sampler, offset.yz, 0).x;
	float sample2 = heightmap.SampleLevel(tex_sampler, offset.xw, 0).x;
	float sample3 = heightmap.SampleLevel(tex_sampler, offset.yw, 0).x;
...
float3 bicubicNormal(float2 texCoords)
{
	texCoords = texCoords * heightMapSize - 0.5;
	...
	float sample0 = heightmap.SampleLevel(tex_sampler, offset.xz, 0).x;	float sample1 = heightmap.SampleLevel(tex_sampler, offset.yz, 0).x;
	float sample2 = heightmap.SampleLevel(tex_sampler, offset.xw, 0).x;	float sample3 = heightmap.SampleLevel(tex_sampler, offset.yw, 0).x;

	//float sample00 = heightmap.SampleLevel(tex_sampler, offset.xz + float2(heightMapTexelSize.x, 0.0f), 0).x;
	//float sample11 = heightmap.SampleLevel(tex_sampler, offset.yz + float2(heightMapTexelSize.x, 0.0f), 0).x;
	//float sample22 = heightmap.SampleLevel(tex_sampler, offset.xw + float2(0.0f, heightMapTexelSize.y), 0).x;
	//float sample33 = heightmap.SampleLevel(tex_sampler, offset.yw + float2(0.0f, heightMapTexelSize.y), 0).x;
...
float3 getHeightData(float2 heightUV)
{
	...
	float a0 = textureBicubic(heightUV + float2(0.0f, 0.0f));
	float a1 = textureBicubic(heightUV + float2(heightMapTexelSize.x, 0.0f));
	float a2 = textureBicubic(heightUV + float2(0.0f, heightMapTexelSize.y));
	return normalFromHeights(float3(a0, a1, a2));

	//float a0 = heightmap.SampleLevel(tex_sampler, heightUV + float2(0.0f, 0.0f), 0);
	//float a1 = heightmap.SampleLevel(tex_sampler, heightUV + float2(heightMapTexelSize.x, 0.0f), 0);
	//float a2 = heightmap.SampleLevel(tex_sampler, heightUV + float2(0.0f, heightMapTexelSize.y), 0);
	//return normalFromHeights(float3(a0, a1, a2));
	...
	float3 n1 = normalFromHeights(float3(a0, a1, b0));
	float3 n2 = normalFromHeights(float3(a1, a2, b1));
	float3 n3 = normalFromHeights(float3(b0, b1, c0));
	float3 n4 = normalFromHeights(float3(b1, b2, c1));
	return (n1 + n2 + n3) / 3;*/
}
...
PSOutput main(PSInput input, bool isFrontFace : SV_IsFrontFace) : SV_Target
{
	...
	float4 albedo = float4(colormap.SampleLevel(tex_sampler, objectUV, 0).xyz, 1.0f);
	...
	if (wireframeMode)
		output.color = float4(float3(0, 1, 0), 1.0f);
	else
