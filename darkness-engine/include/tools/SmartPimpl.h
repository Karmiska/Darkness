...
namespace tools
{
    ...
    public:
        ...
        using unique_ptr_type = engine::unique_ptr<T, deleter_type>;
        ...
        template<class U>
        impl_ptr(engine::unique_ptr<U>&& u,
            typename std::enable_if<
            std::is_convertible<U*, pointer>::value
            && is_default_manageable::value,
            dummy_t_
        ...
        impl_ptr(engine::unique_ptr<U, D>&& u, C&& c,
            typename std::enable_if<
            std::is_convertible<U*, pointer>::value
            && std::is_convertible<D, deleter_type>::value
            && std::is_convertible<C, copier_type>::value,
        ...
        typename std::enable_if<
            ...
            && is_default_manageable::value,
            impl_ptr&
        >::type operator= (engine::unique_ptr<U>&& u) SPIMPL_NOEXCEPT
        {
    ...
    // Helpers to manage unique impl, stored in engine::unique_ptr

    template<class T, class Deleter = void(*)(T*)>
    using unique_impl_ptr = engine::unique_ptr<T, Deleter>;
