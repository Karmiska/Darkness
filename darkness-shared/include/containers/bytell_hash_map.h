...
namespace ska
{
	...
	namespace detailv8
	{
		...
		{
			static constexpr uint8_t magic_for_empty =		static_cast<uint8_t>(0b11111111);
			static constexpr uint8_t magic_for_reserved =	static_cast<uint8_t>(0b11111110);
			static constexpr uint8_t bits_for_direct_hit =	static_cast<uint8_t>(0b10000000);
			static constexpr uint8_t magic_for_direct_hit =	static_cast<uint8_t>(0b00000000);
			static constexpr uint8_t magic_for_list_entry =	static_cast<uint8_t>(0b10000000);

			static constexpr uint8_t bits_for_distance = static_cast<uint8_t>(0b01111111);
			inline static int distance_from_metadata(uint8_t metadata)
			{
		...
		template<typename T>
		constexpr uint8_t sherwood_v8_constants<T>::magic_for_empty;
		template<typename T>
		constexpr uint8_t sherwood_v8_constants<T>::magic_for_reserved;
		template<typename T>
		constexpr uint8_t sherwood_v8_constants<T>::bits_for_direct_hit;
		template<typename T>
		constexpr uint8_t sherwood_v8_constants<T>::magic_for_direct_hit;
		template<typename T>
		constexpr uint8_t sherwood_v8_constants<T>::magic_for_list_entry;

		template<typename T>
		constexpr uint8_t sherwood_v8_constants<T>::bits_for_distance;
		...
		struct sherwood_v8_block
		{
			...
			uint8_t control_bytes[BlockSize];
			union
			{
				T data[BlockSize];
			};
			...
			{
				static std::array<uint8_t, BlockSize> empty_bytes = []
				{
					std::array<uint8_t, BlockSize> result;
			...
			void fill_control_bytes(uint8_t value)
			{
		...
		public:
			...
			iterator end()
			{
				return { entries - 1, 0xffffffffffffffffui64 };
			}
			...
			inline iterator find(const FindKey & key)
			{
				...
				size_t num_slots_minus_ones = this->num_slots_minus_one;
				BlockPointer entriess = this->entries;
				index = hash_policy.index_for_hash(index, num_slots_minus_ones);
				bool first = true;
				for (;;)
				{
					size_t block_index = index / BlockSize;
					int index_in_block = index % BlockSize;
					BlockPointer block = entriess + block_index;
					uint8_t metadata = block->control_bytes[index_in_block];
					if (first)
					{
					...
					if (compares_equal(key, block->data[index_in_block]))
						return { block, index };
					uint8_t to_next_index = metadata & Constants::bits_for_distance;
					if (to_next_index == 0)
						return end();
					index += Constants::jump_distances[to_next_index];
					index = hash_policy.keep_in_range(index, num_slots_minus_ones);
				}
			}
			...
			{
				auto tempA = std::ceil(num_elements / static_cast<double>(_max_load_factor));
				auto temp = num_items > static_cast<size_t>(tempA) ? num_items : static_cast<size_t>(tempA);
				num_items = temp;
				if (num_items == 0)
				{
					reset_to_empty_state();
				...
				for (BlockPointer it = new_buckets, end = new_buckets + old_num_blocks; it != end; ++it)
				{
					for (int i = 0; i < BlockSize; ++i)
					{
						uint8_t metadata = it->control_bytes[i];
			...
			iterator erase(const_iterator begin_it, const_iterator end_it)
			{
				...
				engine::vector<std::pair<int, LinkedListIt>> depth_in_chain;
		...
		private:
			...
			{
				return static_cast<size_t>(std::ceil(_num_elements / static_cast<double>(_max_load_factor)));
			}
			...
			struct LinkedListIt
			{
				...
				uint8_t jump_index() const
				{
					return static_cast<uint8_t>(Constants::distance_from_metadata(metadata()));
				}
				...
				void set_next(uint8_t jump_index)
				{
					uint8_t& metadata = block->control_bytes[index_in_block()];
			...
			SKA_NOINLINE(std::pair<iterator, bool>) emplace_direct_hit(LinkedListIt block, Args&& ... args)
			{
				...
				else
				{
					...
					std::pair<uint8_t, LinkedListIt> free_block = find_free_index(parent_block);
					if (!free_block.first)
					{
						grow();
						return emplace(std::forward<Args>(args)...);
			...
			SKA_NOINLINE(std::pair<iterator, bool>) emplace_new_key(LinkedListIt parent, Args&& ... args)
			{
				...
				std::pair<uint8_t, LinkedListIt> free_block = find_free_index(parent);
				if (!free_block.first)
				{
					grow();
					return emplace(std::forward<Args>(args)...);
			...
			std::pair<uint8_t, LinkedListIt> find_free_index(LinkedListIt parent) const
			{
				for (uint8_t jump_index = 1; jump_index < Constants::num_jump_distances; ++jump_index)
				{
			...
			void grow()
			{
				auto temp = 10 > 2 * bucket_count() ? 10 : 2 * bucket_count();
				rehash(temp);
			}
			...
			void deallocate_data(BlockPointer begin, size_t num_slots_minus_ones)
			{
				if (begin == BlockType::empty_block())
					return;

				++num_slots_minus_ones;
				size_t num_blocks = num_slots_minus_ones / BlockSize;
				if (num_slots_minus_ones % BlockSize)
					++num_blocks;
